...
function Todo() {
  ...
  const handleAdd = () => {
    if (newTodo == "") {
      ...
      localStorage.setItem("todoList", JSON.stringify(updatedAllTodos)); //store array as string not as object
      setNewTodo("");
      setIncompleteTaskCount((prevCount) => prevCount + 1);
    }
  };
  ...
  const handleDelete = (id, isCompleted) => {
    ...
    if (isCompleted) {
      ...
      localStorage.setItem("todoList", JSON.stringify(deletedAllTodos));
      setIncompleteTaskCount((prevCount) => prevCount - 1);
    }
  };
  ...
  const hanldeComplete = (index) => {
    ...
    localStorage.setItem("completedList", JSON.stringify(updatedCompletedArry));
  };
  ...
  useEffect(() => {
    ...
    if (savedTodo) {
      ...
      const initialIncompleteCount = savedTodo.filter(
        (item) => !item.status
      ).length;
      setIncompleteTaskCount(initialIncompleteCount);
    }
  ...
  return (
    <div>
      <div className="headbox">
        <h2>My ToDo-List</h2>
        <h3>Professional</h3>
        <h4>no.of Tasks: {incompleteTaskCount}</h4>
      </div>
